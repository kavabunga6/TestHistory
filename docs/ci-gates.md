...
- `npm run screenshots:capture` builds the web app, starts a local Vite preview, captures synthetic UI PNGs into `docs/screenshots/final`, writes `docs/screenshots/final/manifest.json`, and requires Playwright Chromium to be installed locally. The script tries Playwright's headless shell first and falls back to the regular Chromium executable path when only that browser is present.
