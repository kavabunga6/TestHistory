...
export type PermissionDeniedError = {
  ...
  projectId?: string;
  resource?: {
    type: "project" | "launch" | "test-case" | "test-result" | "artifact" | "defect";
    id?: string;
  };
  traceId?: string;
};

export type DeniedUiSurface = {
  id: "project" | "launch" | "artifact" | "mutation";
  title: string;
  badge: "Готово";
  copy: string;
  actionLabel: string;
...
export type DeniedUiModel = {
  shape: "PermissionDeniedError";
  title: string;
  message: string;
  reason: string;
  resource: string;
  surfaces: DeniedUiSurface[];
};
