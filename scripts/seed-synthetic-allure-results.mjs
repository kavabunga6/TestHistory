...
async function main() {
  ...
  for (let launchIndex = 0; launchIndex < launchPlans.length; launchIndex += 1) {
    ...
    await Promise.all(
      fixtures.files.map((file) => writeFile(path.join(outputDir, file.path), file.content, "utf8"))
    );
...
function buildFixtures({ launchIndex, launchName }) {
  const plan = launchPlans[launchIndex];
  ...
  for (const [caseIndex, caseId] of plan.cases.entries()) {
    ...
    const duration =
      status === "skipped" ? 0 : testCase.baseDuration + launchIndex * 87 + caseIndex * 53;
  ...
  if (status === "skipped") {
    return [
      step(
        ...
        0,
        [],
        [textAttachment(`${runId}/feature-flag.txt`, `${testCase.component}.enabled=false\n`)]
      )
    ];
  }

  const verifyStatus = status === "failed" ? "failed" : status === "broken" ? "broken" : "passed";
  return [
    step("Prepare synthetic preconditions", "passed", start + 100, 220, [
      step("Create generated fixture data", "passed", start + 140, 80),
      step("Open target surface", "passed", start + 230, 70)
    ]),
    step("Execute user journey", verifyStatus, start + 420, 760, [
      step("Submit primary action", status === "broken" ? "broken" : "passed", start + 480, 230),
      step(
        ...
        [
          textAttachment(
            `${runId}/step-validation.log`,
            `validation_status=${verifyStatus}\ncase=${testCase.id}\n`
          )
        ]
      )
    ]),
